...
class SignupForm(FlaskForm):
    ...
    email = wtforms.EmailField("Введіть свою електрону пошту", validators = [wtforms.validators.DataRequired(),wtforms.validators.Email()])
    password = wtforms.PasswordField("Пароль", validators = [wtforms.validators.DataRequired(),wtforms.validators.length(8)])
    sudmit = wtforms.SubmitField("Зареєструватись")
